...
const handleLogin = async (req, res, next) => {
  try {
    ...
    if (!users) {
      return res
        .status(404)
        .json({ message: "User not found with this email", user: users });
    }
    ...
    if (!isPassword) {
      return res.status(401).json({ message: "Did not match password " });
    }
    console.log("reched");
...
const handleLogout = async (req, res, next) => {
  try {
    ...
    return res.status(500).json({ message: " Logout not successful", error });
  }
};
